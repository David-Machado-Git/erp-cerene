...
const reorderedItems = items.value.map(item => {
  ...
  if (file.value) {
    const isCSV = file.value.name.endsWith('.csv'); // Detecta se o arquivo é CSV
    ...
    reader.onload = (event) => {
      ...
      items.value = sheetData.slice(1).map((row) => {
        let obj = {};
        headerRow.forEach((header, index) => {
          let cellValue = row[index];

          if (isCSV && typeof cellValue === 'string' && isPossibleCSVDate(cellValue)) {
            cellValue = handleCSVDate(sheetData); // Formata a data corretamente para CSV
          } else if (typeof cellValue === 'number' && header.toLowerCase().includes('date')) {
            cellValue = convertExcelDate(cellValue); // Converte o número serial para uma data legível
          }
...
function handleCSVDate(dateString) {
  console.log('A DATA É CSV, SEGUE A MESMA: => ', dateString);
  
}


function isPossibleCSVDate(value) {
  return (
    typeof value === 'string' &&
